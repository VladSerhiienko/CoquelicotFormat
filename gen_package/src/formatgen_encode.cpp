...
bool PutLogicalPixelsPacked(char const *name,
	...
	char const UFloatXOp[] = "%s\t\t\t\t*op%d |= (%sCqFormat_FloatToUFloat%dAsUint((float)in[%d])) << %lld;\n";
	...
	for (uint32_t j = 0; j < repeat; j++) {
		...
		for (int i = 0; i < CqFormat_PACK_NUM_CHANNELS; ++i) {
			...
			if (swizzle >= 0 && i < (int)ChannelCount(name, v)) {
				...
				if (load) {
					char tmp[2048];
					loadIndex = (j * CqFormat_PACK_NUM_CHANNELS) + i;
				...
				case CqFormat_PACK_TYPE_SNORM:
					sprintf(output,
									...
									output,
									loadIndex,
									(type == CqFormat_PACK_TYPE_UNORM) ? castBuffer : scastBuffer,
									j * 4 + swizzle,
									maxFactor,
									Mask(chanBitWidth),
									shifter);
				...
				case CqFormat_PACK_TYPE_UINT:
					sprintf(output,
									...
									output,
									loadIndex,
									(type == CqFormat_PACK_TYPE_UINT) ? castBuffer : scastBuffer,
									j * 4 + swizzle,
									Mask(chanBitWidth),
									shifter);
					break;
				case CqFormat_PACK_TYPE_SRGB:
					sprintf(output,
									"%s\t\t\t\t*op%d |= (%sCqFormat_FloatToSRGB((float)in[%d])) << %lld;\n",
									output,
									loadIndex,
									castBuffer,
									j * 4 + swizzle,
				...
				case CqFormat_PACK_TYPE_SFLOAT:
					if (chanBitWidth == 16) {
						sprintf(output,
											"%s\t\t\t\t*op%d |= (%sCqFormat_FloatToHalfAsUint((float)in[%d])) << %lld;\n",
											output,
											loadIndex,
											castBuffer,
											j * 4 + swizzle,
				...
				case CqFormat_PACK_TYPE_SBFLOAT:
					if (chanBitWidth == 16) {
						sprintf(output,
										"%s\t\t\t\t*op%d |= (%sCqFormat_FloatToBFloatAsUint((float)in[%d])) << %lld;\n",
										output,
										loadIndex,
										castBuffer,
										j * 4 + swizzle,
...
#define  CqFormat_MOD_MACRO(x, y) { bool okay = EncodeLogicalPixelsF(#x, y, output); if(okay) { sprintf(buffer, body, "CqFormat_"#x); VFile_Write(file, buffer, strlen(buffer)); } }
...
void GenEncodeLogicalPixelsF(VFile_Handle file) {
	...
	char output[2048];
#define  CqFormat_START_MACRO VFile_Write(file, isPrefixF, strlen(isPrefixF));
#define  CqFormat_MOD_MACRO(x, y) { bool okay = EncodeLogicalPixelsF(#x, y, output); if(okay) { sprintf(buffer, body, "CqFormat_"#x, output); VFile_Write(file, buffer, strlen(buffer)); } }
#define  CqFormat_END_MACRO sprintf(buffer, switchPostfixF); VFile_Write(file, buffer, strlen(buffer));
#include "formatgen.h"
}
...
static void GenCanEncodeLogicalPixelsD(VFile_Handle file) {
	...
	char const *const body = "\t\tcase %s: return true;\n";
	char const switchPostfixF[] = "\t\tdefault: return CqFormat_CanEncodeLogicalPixelsF(fmt);\n"
																"\t}\n}\n\n";

	char output[2048];
#define  CqFormat_START_MACRO VFile_Write(file, isPrefixF, strlen(isPrefixF));
#define  CqFormat_MOD_MACRO(x, y) { bool okay = EncodeLogicalPixelsD(#x, y, output); if(okay) { sprintf(buffer, body, "CqFormat_"#x); VFile_Write(file, buffer, strlen(buffer)); } }
#define  CqFormat_END_MACRO sprintf(buffer, switchPostfixF); VFile_Write(file, buffer, strlen(buffer));
#include "formatgen.h"
...
void GenEncodeLogicalPixelsD(VFile_Handle file) {
	...
	char output[2048];
#define  CqFormat_START_MACRO VFile_Write(file, isPrefixF, strlen(isPrefixF));
#define  CqFormat_MOD_MACRO(x, y) { bool okay = EncodeLogicalPixelsD(#x, y, output); if(okay) { sprintf(buffer, body, "CqFormat_"#x, output); VFile_Write(file, buffer, strlen(buffer)); } }
#define  CqFormat_END_MACRO sprintf(buffer, switchPostfixF); VFile_Write(file, buffer, strlen(buffer));
#include "formatgen.h"
}
